...
void init_WCAFC_interaction(vector<OpenMM::CustomNonbondedForce*> &WCAFCs,
    ...
    set<int> class_set;
    for (int ic=0; ic<set_1.size(); ic++) {
        for (int i=0; i<set_1[ic].size(); i++) {
            class_set.insert(set_1[ic][i].begin(),set_1[ic][i].end());
        }
    }
    

    set<int> :: iterator it_1 = class_set.begin();
    set<int> :: iterator it_2 = set_2[set_2_index].begin();

    string epsilon = to_string(generalParameters.BoltzmannKJpermolkelvin*generalParameters.temperature) ;
    string sigma;
    if (use_max_radius) {
        sigma = to_string(max( atoms[*it_1].radius, atoms[*it_2].radius ));
    } else {
        sigma = to_string(atoms[*it_1].radius + atoms[*it_2].radius) ;
    }
...
void init_WCAFC_interaction(vector<OpenMM::CustomNonbondedForce*> &WCAFCs,
    ...
    for (int ic=0; ic<class_set.size(); ic++) {
        for (int i=0; i<class_set[ic].size(); i++) {
            class_set1D.insert(class_set[ic][i].begin(),class_set[ic][i].end());
        }
    }
    

    set<int> :: iterator it = class_set1D.begin();
