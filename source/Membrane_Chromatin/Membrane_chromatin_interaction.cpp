...
void Chromatin_Membrane_hard_sphere(Chromatin &chromo, Membrane &Mem){
    ...
    for (int i=0; i<chromo.get_num_of_nodes(); i++) {
        ...
        for (int j=0; j<chromo.Membrane_neighbour_node[i].size(); j++) {
            ...
            deltax=chromo.get_node_position(Node_B, 0)-Mem.get_node_position(Node_A, 0);
            deltay=chromo.get_node_position(Node_B, 1)-Mem.get_node_position(Node_A, 1);
            deltaz=chromo.get_node_position(Node_B, 2)-Mem.get_node_position(Node_A, 2);
...
void Chromatin_Membrane_neighbour_finder(Chromatin& chromo, Membrane Mem){
    ...
    chromo.Membrane_neighbour_node.resize(chromo.get_num_of_nodes());
    double threshold_dist=(chromo.get_node_radius()+Mem.Average_node_pair_length);
    for (int i=0; i<chromo.get_num_of_nodes(); i++) {
        for (int j=0; j<Mem.get_num_of_nodes(); j++) {
            
            double delta_x=chromo.get_node_position(i, 0)-Mem.get_node_position(j, 0);
            double delta_y=chromo.get_node_position(i, 1)-Mem.get_node_position(j, 1);
            double delta_z=chromo.get_node_position(i, 2)-Mem.get_node_position(j, 2);
