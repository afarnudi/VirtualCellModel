...
ldata, time = import_trajectories_from_pdb(longname)

lr2n = R2N (ldata[ini_t:fin_t,mem_nodes:,:], 'Long'  ,ax2)
...
lN = lr2n.shape
lNbp =[]
for i in range(1,lN[0]*10+1,10):
    lNbp.append(i*0.01575)
...
lr2ntomum    = lr2ntomum*scale*scale

print(lr2ntomum.shape)
print(lNbp)
...
ax1.errorbar(lNbp,lr2ntomum, label='Long-'+str(scale), marker='s', c='m'    , ms=2, ls='none', zorder=6 )
...
#ax1.plot(datawlcN,datawlc,'black',label='WLC',zorder=9)
#ax1.plot(datawlcN,dataring,'r',label='Ring', zorder=10)
